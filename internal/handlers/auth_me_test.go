...
func TestAuthMeAllowsGetPatchAndOptions(t *testing.T) {
	...
	if got := options.Header().Get("Access-Control-Allow-Methods"); got != "GET, PATCH, OPTIONS" {
	...
	if got := post.Header().Get("Allow"); got != "GET, PATCH, OPTIONS" {
