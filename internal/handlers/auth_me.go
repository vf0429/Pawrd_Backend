...
// NewAuthMeHandler returns or updates the current account truth for an
// authenticated session. GET keeps the mainline recovery payload while PATCH
// delegates to the unified-profile write-through handler.
func NewAuthMeHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodPatch {
			NewAuthMeUpdateHandler().ServeHTTP(w, r)
			return
		}
