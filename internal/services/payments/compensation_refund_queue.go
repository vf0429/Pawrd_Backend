...
func EnsureSystemCompensationRefund(
	...
	if strings.TrimSpace(order.ID) == "" ||
		strings.TrimSpace(order.PaymentIntentID) == "" ||
	...
	idempotencyKey := systemCompensationIdempotencyKey(reason, order.PaymentIntentID)
...
func (q *DurableCompensationRefundQueue) executeRefund(
	...
	err := q.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if strings.TrimSpace(order.PaymentIntentID) != strings.TrimSpace(refund.PaymentIntentID) {
