...
func (d *Dispatcher) reconcileShopifyOrder(
	...
	result, err := lookup.FindOrderBySourceIdentifier(ctx, order.PaymentIntentID)
	if err != nil {
		return false, err
	}
	if result == nil {
...
func (d *Dispatcher) shopifyOrderInput(order models.ShopOrder) (shopify.AdminOrderInput, error) {
	...
	case err == nil:
		...
		if strings.TrimSpace(quote.PaymentIntentID) != strings.TrimSpace(order.PaymentIntentID) ||
